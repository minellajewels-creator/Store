...
@media(max-width:600px){.topbar-nav{gap:14px}.topbar-nav a{font-size:10px;letter-spacing:0.3px}}
...
.hero h1{font-family:'Libre Baskerville',serif;font-size:clamp(32px,7vw,66px);font-weight:400;line-height:1.05;color:var(--plum);margin-bottom:12px;animation:fadeSlideUp 0.8s cubic-bezier(0.22,1,0.36,1) both;animation-delay:0.25s}
.hero h1 em{font-style:italic;color:var(--gold)}
.hero-sub{font-size:clamp(13px,2.5vw,16px);color:var(--muted);font-weight:300;margin-bottom:20px;max-width:360px;animation:fadeSlideUp 0.8s cubic-bezier(0.22,1,0.36,1) both;animation-delay:0.4s}
.hero-cod-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(46,125,79,0.12);border:1px solid rgba(46,125,79,0.25);border-radius:20px;padding:5px 14px;font-size:11px;color:var(--ok);font-weight:500;margin-bottom:22px;animation:fadeSlideUp 0.8s cubic-bezier(0.22,1,0.36,1) both;animation-delay:0.5s}
...
.cat-bar{position:sticky;top:calc(60px + var(--safe-top));z-index:400;background:rgba(250,246,240,0.95);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-bottom:1px solid var(--border);display:flex;justify-content:flex-start;gap:6px;padding:10px 16px;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;-webkit-scroll-snap-type:none}
@media(min-width:600px){.cat-bar{justify-content:center}}
...
.pview-thumb-vid.active{border-color:var(--gold);opacity:1}
.pview-carousel{position:relative;width:100%;height:100%;overflow:hidden}
.pview-carousel-track{display:flex;height:100%;transition:transform .38s cubic-bezier(.4,0,.2,1)}
.pview-carousel-slide{flex-shrink:0;height:100%;background:var(--cream2);display:flex;align-items:center;justify-content:center;position:relative}
...
function initCardAnimations(){
  ...
  var io=new IntersectionObserver(function(entries){
    entries.forEach(function(e,i){
      if(e.isIntersecting){setTimeout(function(){e.target.classList.add("card-visible");},i*60);io.unobserve(e.target);}
    });
  },{threshold:0.08});
